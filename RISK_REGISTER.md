...
| 2 | **Source closes its doors** (WAF like the US/AU/IE ones, or robots change) | MEDIUM (it already happened to candidate sources and UDI) | HIGH | UDI is now the tested recovery case: remove it from active collection, retain its append-only evidence with an explicit `source_unavailable` state, freeze the last successful verification, and keep all remaining active sources fail-closed. Playbooks in runbook; replacement sources remain queued. |
