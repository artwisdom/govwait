...
45. Limited the first Norway source to UDI's five complete server-rendered waiting-time tables (19 routes); personalised question-and-answer guides remain excluded because their output depends on user selections and hidden combinations must not be guessed or probed.
46. Preserve a UDI range such as `15–29 days` verbatim in `value_raw`, while using the upper endpoint only as the conservative normalized `value_days` for sorting and comparison; the UI explains that normalization and never turns the range into a single official promise.
47. Kept the 2,318-entity / 2,104-page / 635-sitemap Norway candidate local after all release gates passed. Owner approval, commit/push, production deployment, public-edge verification, and discovery submission remain separate evidence gates.
